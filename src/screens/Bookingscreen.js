...
function Bookingscreen(props) {
    ...
    const getData = async () => {
        try {
            setloading(true)
            const data = (await axios.post('http://localhost:4000/api/rooms/getroombyid', { roomid: roomid })).data;
    ...
    async function bookRoom() {
        ...
        try {
            ...
            setloading(true)
            const result = await axios.post('http://localhost:4000/api/bookings/bookroom', bookingDetails);
            setloading(false);
            console.log(result);
